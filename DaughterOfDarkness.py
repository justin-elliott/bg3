...
class DaughterOfDarkness(ProgressionReplacer):
    ...
    def __init__(self):
        super().__init__(os.path.dirname(__file__),
                         ...
                         classes=CharacterSubclasses.CLERIC)
    ...
    @class_level(CharacterClass.CLERIC, range(2, 13))
    def level_2_to_12_cleric(self, progression: Progression) -> None:
    ...
    @class_level(CharacterClass.CLERIC_TEMPEST, 11)
    ...
    def postprocess(self, progressions: Iterable[Progression]) -> None:
        multiply_resources(progressions, [ActionResource.SPELL_SLOTS, ActionResource.CHANNEL_DIVINITY_CHARGES], 2)

    def make_progression(self, character_class: CharacterClass, level: int) -> Progression:
        assert character_class == CharacterClass.CLERIC_TEMPEST
        return Progression(
            Name=str(character_class),
            Level=level,
            ProgressionType=1,
            TableUUID="0630b55b-36f4-41d2-aacc-28e99d6ae252",
            UUID=self.make_uuid(f"Tempest Domain Cleric Level {level}")
        )
