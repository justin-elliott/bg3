...
with open(os.path.join(base_dir, "Public", "Serenade", "Stats", "Generated", "Data", "Medley.txt"), "w") as f:
    ...
    f.write(textwrap.dedent(f"""\
        ...
        data "SpellProperties" "ApplyStatus(SERENADE_MEDLEY,100,-1);ApplyStatus(LONGSTRIDER,100,-1);ApplyStatus(PETPAL,100,-1);IF(not WearingArmor()):ApplyStatus(MAGE_ARMOR,100,-1);IF(not Tagged('UNDEAD') and not Tagged('CONSTRUCT')):RegainHitPoints(LevelMapValue(Serenade_HealValue)+SpellCastingAbilityModifier)"
        data "TargetConditions" "Character() and Ally() and not Dead()"
        data "TooltipDamageList" "RegainHitPoints(LevelMapValue(Serenade_HealValue)+SpellCastingAbilityModifier)"
        ...
        data "DescriptionParams" "LevelMapValue(Serenade_AidValue);Distance(24)"
        data "Icon" "Action_Song_SingForMe"
        data "StackId" "AID"
